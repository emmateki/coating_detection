...
def generate_mask(
    ...
    # Save binary mask as an image for reference
    imageio.imwrite(
        "/home/emma/Documents/UJV/CORROSION/coating/fiji_model/binary_mask.png",
        (binary_mask * 255).astype(np.uint8),
    )

    # Find clusters touching both sides
    final_mask = find_cluster_touches_sides(labeled_mask)
    if final_mask is not None:
        return final_mask

    # Check for clusters on the left and right sides
    left_mask = find_largest_clusters(labeled_mask, "left")
    right_mask = find_largest_clusters(labeled_mask, "right")

    # Combine masks if both sides have significant clusters
    if left_mask is not None and right_mask is not None:
        combined_mask = np.logical_or(left_mask, right_mask)
    elif left_mask is not None:
        combined_mask = left_mask
    elif right_mask is not None:
        combined_mask = right_mask
    else:
        # If no clusters are found on either side, return the largest overall cluster
        largest_cluster = find_largest_clusters(labeled_mask, "all")
        combined_mask = largest_cluster if largest_cluster is not None else binary_mask
